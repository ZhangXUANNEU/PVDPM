...
def total_loss(Q, P, A, train, Graphs):
    ...
    if train == 1:
        Q_mean = Q.mean(dim=1)  # [batch, seg_len]

        mask_Q = (label_Qt != 0)
        mask_P = (label_Pt != 0)
        mask_A = (label_At != 0)

        Diff_Q = ((Q_mean - label_Qt) / label_Qt)[mask_Q]
        Diff_P = ((P - label_Pt) / label_Pt)[mask_P]
        Diff_A = ((A - label_At) / label_At)[mask_A]
